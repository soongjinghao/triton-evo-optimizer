...
def fused_gdn_gating_kernel(
    ...
    """
    Optimized for Ascend NPU with better cache locality:
    - Process BLK_BATCH batches per thread block for better L2 cache utilization
    - Access memory in contiguous 128B cache lines (32 float32 elements)
    - Reduce strided memory access by processing multiple batches together
    """
...
def fused_gdn_gating(
    ...
    Optimized for Ascend NPU with better cache locality and memory access patterns.
    """
    ...
    BLK_HEADS = min(BLK_HEADS, triton.next_power_of_2(num_heads))
    BLK_HEADS = max(16, BLK_HEADS)
    ...
    BLK_BATCH = min(BLK_BATCH, triton.next_power_of_2(batch))
    BLK_BATCH = max(8, BLK_BATCH)
