...
def _compute_pid_swizzled(tile_id, num_pid_m, num_pid_n, GROUP_SIZE_M: tl.constexpr):
    """Compute swizzled tile coordinates for better cache locality."""
...
def matmul_kernel_simplified(
    ...
    """
    Simplified matmul kernel - single loop over tiles.
    
    Key simplifications:
    1. No persistent outer loop - one program per tile
    2. Proper masking instead of clamping
    3. Cleaner pointer arithmetic
    4. Removed unnecessary optimization hints
    """
    # Get tile ID and compute (pid_m, pid_n)
    tile_id = tl.program_id(axis=0)
    num_pid_m = tl.cdiv(M, BLOCK_SIZE_M)
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    
    # Use swizzled pattern for cache locality
    ...
    for ki in range(k_tiles):
        ...
        # Create proper masks (don't clamp indices!)
...
def matmul_persistent(
    ...
    # Optimization 1: Set NUM_SMS to match actual AICore count on Ascend NPU (24 cores)
    ...
    # Optimization 5: Retune block sizes for Ascend NPU characteristics
    configs = {
        torch.bfloat16: {
            "BLOCK_SIZE_M": 128,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 32,  # Reduced for better memory utilization on NPU
            "GROUP_SIZE_M": 8,
            "num_stages": 3,
            "num_warps": 8,
        },
        torch.float16: {
            "BLOCK_SIZE_M": 128,
            "BLOCK_SIZE_N": 128,  # Reduced from 256 for better cache utilization
            "BLOCK_SIZE_K": 32,   # Reduced for better memory utilization on NPU
            "GROUP_SIZE_M": 8,
            "num_stages": 3,
            "num_warps": 8,
        ...
        torch.float32: {
            "BLOCK_SIZE_M": 64,   # Reduced for better memory utilization
            "BLOCK_SIZE_N": 64,   # Reduced for better memory utilization
            "BLOCK_SIZE_K": 64,
            "GROUP_SIZE_M": 8,
            "num_stages": 3,
            "num_warps": 8,
