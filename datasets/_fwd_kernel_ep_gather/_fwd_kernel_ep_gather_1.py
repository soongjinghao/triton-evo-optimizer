...
def _fwd_kernel_ep_gather(
    ...
    # Stride-stripped contiguous offset for hidden dimension
            ...
            if b_token < total_token_num:
                ...
                for topk_index in range(0, topk_num):
                    ...
                    if expert_id >= 0:
                        ...
                        # Contiguous 128-bit aligned load for input block
                        in_base = input_tensor + source_token_index * input_tensor_stride0 + d_block * BLOCK_D
                        in_ptr = tl.multiple_of(in_base, 16)
                        tmp = tl.load(in_ptr + off_d)

                        accumulator += tmp.to(tl.float32) * acc_weight

                # Contiguous 128-bit aligned store for output block
                out_base = output_tensor + b_token * output_tensor_stride0 + d_block * BLOCK_D
                out_ptr = tl.multiple_of(out_base, 16)
...
def ep_gather(
    ...
    # Enforce contiguous innermost dimension for input and output
