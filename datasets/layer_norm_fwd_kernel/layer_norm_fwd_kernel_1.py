...
def layer_norm_fwd_kernel(
    ...
    row_start = row_block_id * ROWS_PER_BLOCK
    rows = row_start + tl.arange(0, ROWS_PER_BLOCK)
    row_mask = rows < M
    const_off = group * N

    if N > BLOCK_N:
        # 分块规约路径:大 group_size 时逐块加载、累加,再归一化写回
        ...
        for col_start in range(0, N, BLOCK_N):
            ...
            X_base = X + rows[:, None] * stride_x_row + cols[None, :] + const_off
            x = tl.load(X_base, mask=mask, other=0.0).to(tl.float32)
            if HAS_Z and not NORM_BEFORE_GATE:
                Z_base = Z + rows[:, None] * stride_z_row + cols[None, :] + const_off
        ...
        for col_start in range(0, N, BLOCK_N):
            ...
            Y_base = Y + rows[:, None] * stride_y_row + cols[None, :] + const_off
            tl.store(Y_base, y, mask=mask)
    else:
        # 小 group_size 整行一次加载的原路径
...
def calc_rows_per_block(
    ...
    # Keep the proven public/small-N scheduling unchanged.
    if BLOCK_N <= 256:
        row_cap = 32
    elif BLOCK_N <= 512:
        row_cap = 16
    ...
    # Physical tile resource cap.
    max_rows_by_tile = max(MAX_FUSED_SIZE // BLOCK_N, 1)
    rows_per_block = min(rows_per_block, max_rows_by_tile)

    # IMPORTANT hidden-shape guard:
    # when LOGICAL_N > 8192 the kernel processes the row in multiple BLOCK_N
    # chunks. Capping only by BLOCK_N incorrectly allows 8 rows/program even
    # for a 16K/32K logical row, making each program execute a very long
    # two-pass serial loop. Cap by the full logical group size as well.
...
def layer_norm_fwd(
    ...
    num_row_blocks = cdiv(M, rows_per_block)
    grid = (num_row_blocks, ngroups)
