...
def fn_triton_kernel(
    ...
    # 可证明不变量:k 的 token/head 基地址在两次写入间复用
    token_k_base = token_id * K_STRIDE_0
    head_offs_k = head_id * K_STRIDE_1
    k_base = token_k_base[:, None, None] + head_offs_k[None, :, None]

    token_nope_base = token_id * K_NOPE_STRIDE_0
    head_offs_nope = head_id * K_NOPE_STRIDE_1
    offs_nope = (
        token_nope_base[:, None, None]
    ...
    tl.store(
        k_ptr + k_base + nope_sub_id[None, None, :],
        vals_nope,
        mask=mask,
    )

    token_rope_base = token_id * K_ROPE_STRIDE_0
    offs_rope = token_rope_base[:, None, None] + rope_sub_id[None, None, :]
    vals_rope = tl.load(k_rope_ptr + offs_rope, mask=token_mask[:, None, None])

    tl.store(
        k_ptr + k_base + rope_sub_id[None, None, :] + QK_NOPE_HEAD_DIM,
        vals_rope,
        mask=mask,
    )
