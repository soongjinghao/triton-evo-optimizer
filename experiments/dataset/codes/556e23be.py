...
def reshape_and_cache_flash_kernel(
    ...
    if head_size <= CHUNK_ELEMENTS:
        ...
        for h0 in range(0, num_heads, BH):
            ...
            load_mask = head_mask[:, None] & valid
            k = tl.load(key_base + h0 * head_size + head_offsets, mask=load_mask)
            v = tl.load(value_base + h0 * head_size + head_offsets, mask=load_mask)
            tl.store(key_cache_base + h0 * head_size + head_offsets, k, mask=load_mask)
            tl.store(value_cache_base + h0 * head_size + head_offsets, v, mask=load_mask)
    else:
