...
@triton.jit
def _fused_qkvzba_split_reshape_cat_row_kernel(
    mixed_qkv,
    z,
    b,
    a,
    ...
    V_SIZE: tl.constexpr = V_PER_QK * HEAD_V

    i_bs_block = tl.program_id(0)

    offs_i = tl.arange(0, NUM_HEADS_QK)[:, None]
    offs_q = tl.arange(0, HEAD_QK)[None, :]
    offs_v = tl.arange(0, V_SIZE)[None, :]
    offs_b = tl.arange(0, V_PER_QK)[None, :]

    for b_offset in range(BLOCK_B):
        b_idx = i_bs_block * BLOCK_B + b_offset
        if b_idx < TOTAL_BATCH:
            row_qkvz_base = b_idx * NUM_HEADS_QK * QKVZ_DIM_T
            row_ba_base = b_idx * NUM_HEADS_QK * BA_DIM_T

            # q
            q_vals = tl.load(mixed_qkvz + row_qkvz_base + offs_i * QKVZ_DIM_T + offs_q)
            tl.store(mixed_qkv + b_idx * QKV_DIM_T + offs_i * HEAD_QK + offs_q, q_vals)

            # k
            k_vals = tl.load(mixed_qkvz + row_qkvz_base + offs_i * QKVZ_DIM_T + HEAD_QK + offs_q)
            tl.store(mixed_qkv + b_idx * QKV_DIM_T + NUM_HEADS_QK * HEAD_QK + offs_i * HEAD_QK + offs_q, k_vals)

            # v
            v_vals = tl.load(mixed_qkvz + row_qkvz_base + offs_i * QKVZ_DIM_T + HEAD_QK * 2 + offs_v)
            tl.store(mixed_qkv + b_idx * QKV_DIM_T + NUM_HEADS_QK * HEAD_QK * 2 + offs_i * V_SIZE + offs_v, v_vals)

            # z
            z_vals = tl.load(mixed_qkvz + row_qkvz_base + offs_i * QKVZ_DIM_T + HEAD_QK * 2 + V_SIZE + offs_v)
            tl.store(z + b_idx * NUM_HEADS_V * HEAD_V + offs_i * V_SIZE + offs_v, z_vals)

            # b
            b_vals = tl.load(mixed_ba + row_ba_base + offs_i * BA_DIM_T + offs_b)
            tl.store(b + b_idx * NUM_HEADS_V + offs_i * V_PER_QK + offs_b, b_vals)

            # a
            a_vals = tl.load(mixed_ba + row_ba_base + offs_i * BA_DIM_T + V_PER_QK + offs_b)
            tl.store(a + b_idx * NUM_HEADS_V + offs_i * V_PER_QK + offs_b, a_vals)
...
def fused_qkvzba_split_reshape_cat(
    ...
    num_heads_v_per_qk = num_heads_v // num_heads_qk
    qkvz_dim_t = head_qk * 2 + num_heads_v_per_qk * head_v * 2
    ba_dim_t = num_heads_v_per_qk * 2
    row_qkvz_elems = num_heads_qk * qkvz_dim_t
    row_ba_elems = num_heads_qk * ba_dim_t

    # Shape guard: only apply full-row consolidation when the per-row working set
    # is known to be small enough to avoid UB/register overflow.
    if row_qkvz_elems <= 4096 and row_ba_elems <= 512:
        grid = (triton.cdiv(TOTAL_BATCH, BLOCK_B),)
        _fused_qkvzba_split_reshape_cat_row_kernel[grid](
            mixed_qkv,
            z,
            b,
            a,
