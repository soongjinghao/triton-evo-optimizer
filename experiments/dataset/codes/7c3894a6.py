...
def merge_16x16_to_32x32_inverse_kernel(
    ...
    m_A = o_i_f32[:, None] > o_i_f32[None, :]
    m_I = o_i_f32[:, None] == o_i_f32[None, :]
    A += (bos * H + i_h) * BT
    Ai += (bos * H + i_h) * BT
    ...
    loop_bound_1 = min(16, T - i_t * BT)
    loop_bound_2 = min(32, T - i_t * BT)
    
    for i in range(2, loop_bound_1):

        i_f32 = i * 1.0
        b_a_11 = -tl.load(A + (i_t * BT + i) * H * BT + o_i)
        b_a_11 += tl.sum(b_a_11[:, None] * b_Ai_11, 0)
        cmp_mask = o_i_f32 == i_f32
        b_Ai_11 = tl.where(cmp_mask[:, None], b_a_11, b_Ai_11)
    
    for i in range(16 + 2, loop_bound_2):

        i_f32 = (i - 16) * 1.0
        b_a_22 = -tl.load(A + (i_t * BT + i) * H * BT + o_i + 16)
        b_a_22 += tl.sum(b_a_22[:, None] * b_Ai_22, 0)
        cmp_mask = o_i_f32 == i_f32
        b_Ai_22 = tl.where(cmp_mask[:, None], b_a_22, b_Ai_22)
