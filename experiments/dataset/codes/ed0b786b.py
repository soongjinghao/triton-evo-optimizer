...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    batch_offset = batch_idx * block_table_stride
    page_start = group_idx * BLOCK_PAGES

    dequant_dtype = mock_kv_cache_ptr.dtype.element_ty
    k_scale_val = tl.load(k_scale_ptr)
    v_scale_val = tl.load(v_scale_ptr)
    offs = tl.arange(0, K_CACHE_STRIDE)

    for i in range(BLOCK_PAGES):
        page_idx = page_start + i
        if page_idx < block_table_stride:
            orig_page_num = tl.load(
                block_tables_prefill_ptr + batch_offset + page_idx
            ).to(tl.int64)
            if orig_page_num > 0:
                orig_base = orig_page_num * KV_CACHE_STRIDE
                mock_page_num = batch_offset + page_idx + 1
                mock_base = mock_page_num * KV_CACHE_STRIDE

                fp8_vals = tl.load(kv_cache_ptr + orig_base + offs)
                dequantized_vals = (fp8_vals.to(tl.float32) * k_scale_val).to(dequant_dtype)
                tl.store(mock_kv_cache_ptr + mock_base + offs, dequantized_vals)

                fp8_vals = tl.load(kv_cache_ptr + orig_base + K_CACHE_STRIDE + offs)
                dequantized_vals = (fp8_vals.to(tl.float32) * v_scale_val).to(dequant_dtype)
                tl.store(mock_kv_cache_ptr + mock_base + K_CACHE_STRIDE + offs, dequantized_vals)
...
def trtllm_prefill_attn_kvfp8_dequant(
    ...
    BLOCK_PAGES = 4
    num_groups = (num_of_page_per_token + BLOCK_PAGES - 1) // BLOCK_PAGES
