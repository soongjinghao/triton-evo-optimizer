...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    out_page_idx = batch_idx * block_table_stride + mock_block_table_idx + 1
    src_base = orig_page_num * KV_CACHE_STRIDE
    dst_base = out_page_idx * KV_CACHE_STRIDE

    src_k_offsets = src_base + tl.arange(0, K_CACHE_STRIDE)
    fp8_vals_k = tl.load(tl.multiple_of(kv_cache_ptr + src_k_offsets, 16))
    dequantized_k = fp8_vals_k.to(tl.float32) * k_scale_val
    dst_k_offsets = dst_base + tl.arange(0, K_CACHE_STRIDE)
    tl.store(tl.multiple_of(mock_kv_cache_ptr + dst_k_offsets, 16), dequantized_k.to(dequant_dtype))

    src_v_offsets = src_base + K_CACHE_STRIDE + tl.arange(0, K_CACHE_STRIDE)
    fp8_vals_v = tl.load(tl.multiple_of(kv_cache_ptr + src_v_offsets, 16))
    dequantized_v = fp8_vals_v.to(tl.float32) * v_scale_val
    dst_v_offsets = dst_base + K_CACHE_STRIDE + tl.arange(0, K_CACHE_STRIDE)
    tl.store(tl.multiple_of(mock_kv_cache_ptr + dst_v_offsets, 16), dequantized_v.to(dequant_dtype))
