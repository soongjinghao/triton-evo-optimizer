...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    offs = tl.arange(0, KV_CACHE_STRIDE)
    kv_offs = orig_page_num * KV_CACHE_STRIDE + offs
    fp8_vals = tl.load(kv_cache_ptr + kv_offs)

    scale_vals = tl.where(offs < K_CACHE_STRIDE, k_scale_val, v_scale_val)
    dequantized_vals = fp8_vals.to(tl.float32) * scale_vals
    dequantized_vals = dequantized_vals.to(dequant_dtype)

    mock_cache_offset = (
        batch_idx * block_table_stride + mock_block_table_idx + 1
    ) * KV_CACHE_STRIDE + offs
