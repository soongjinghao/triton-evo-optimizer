...
def _selective_scan_update_kernel(
    ...
    if stride_state_dstate == 1 and stride_A_dstate == 1:
    ...
    if not TIE_HDIM:
        ...
        dt_vec = tl.full((BLOCK_SIZE_M,), dt, tl.float32)
        if DT_SOFTPLUS:
            dt_vec = softplus(dt_vec)
        A = tl.load(A_ptr).to(tl.float32)
        dA_vec = tl.exp(tl.full((BLOCK_SIZE_M,), A, tl.float32) * dt_vec)
        dA = dA_vec[:, None]
