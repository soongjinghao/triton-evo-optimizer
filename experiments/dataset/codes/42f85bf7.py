...
@triton.jit
def fused_qkvzba_split_reshape_cat_kernel_2d(
    mixed_qkv,
    z,
    b,
    a,
    ...
    for h_offset in range(BLOCK_H):
        head_idx = head_group * BLOCK_H + h_offset
        if head_idx < NUM_HEADS_QK:
            for b_offset in range(BLOCK_B):
                b_idx = bs_block * BLOCK_B + b_offset
                if b_idx < TOTAL_BATCH:
                    base_ptr = mixed_qkvz + b_idx * NUM_HEADS_QK * QKVZ_DIM_T + head_idx * QKVZ_DIM_T
                    q_end: tl.constexpr = HEAD_QK
                    k_end: tl.constexpr = q_end + HEAD_QK
                    v_end: tl.constexpr = k_end + NUM_HEADS_V_PER_QK * HEAD_V
                    z_end: tl.constexpr = v_end + NUM_HEADS_V_PER_QK * HEAD_V
                    ...
                    v_vals = tl.load(base_ptr + tl.arange(k_end, v_end))
                    z_vals = tl.load(base_ptr + tl.arange(v_end, z_end))
                    blk_q_st_ptr = mixed_qkv + b_idx * NUM_HEADS_QK * QKV_DIM_T + head_idx * HEAD_QK
                    blk_k_st_ptr = mixed_qkv + b_idx * NUM_HEADS_QK * QKV_DIM_T + NUM_HEADS_QK * HEAD_QK + head_idx * HEAD_QK
                    blk_v_st_ptr = mixed_qkv + b_idx * NUM_HEADS_QK * QKV_DIM_T + NUM_HEADS_QK * HEAD_QK * 2 + head_idx * HEAD_V * NUM_HEADS_V_PER_QK
                    tl.store(blk_q_st_ptr + tl.arange(0, HEAD_QK), q_vals)
                    tl.store(blk_k_st_ptr + tl.arange(0, HEAD_QK), k_vals)
                    tl.store(blk_v_st_ptr + tl.arange(0, HEAD_V * NUM_HEADS_V_PER_QK), v_vals)
                    blk_z_st_ptr = z + b_idx * NUM_HEADS_V * HEAD_V + head_idx * HEAD_V * NUM_HEADS_V_PER_QK
                    tl.store(blk_z_st_ptr + tl.arange(0, HEAD_V * NUM_HEADS_V_PER_QK), z_vals)
                    ba_base_ptr = mixed_ba + b_idx * NUM_HEADS_QK * BA_DIM_T + head_idx * BA_DIM_T
                    b_end: tl.constexpr = NUM_HEADS_V_PER_QK
                    a_end: tl.constexpr = b_end + NUM_HEADS_V_PER_QK
                    b_vals = tl.load(ba_base_ptr + tl.arange(0, b_end))
                    a_vals = tl.load(ba_base_ptr + tl.arange(b_end, a_end))
                    blk_b_st_ptr = b + b_idx * NUM_HEADS_V + head_idx * NUM_HEADS_V_PER_QK
                    blk_a_st_ptr = a + b_idx * NUM_HEADS_V + head_idx * NUM_HEADS_V_PER_QK
                    tl.store(blk_b_st_ptr + tl.arange(0, NUM_HEADS_V_PER_QK), b_vals)
                    tl.store(blk_a_st_ptr + tl.arange(0, NUM_HEADS_V_PER_QK), a_vals)
def fused_qkvzba_split_reshape_cat(
    ...
    BLOCK_B = 2
    BLOCK_H = 2 if num_heads_qk > 1 else 1
    TOTAL_BATCH = batch * seq_len
    grid = (
        triton.cdiv(TOTAL_BATCH, BLOCK_B),
        triton.cdiv(num_heads_qk, BLOCK_H),
    )
    fused_qkvzba_split_reshape_cat_kernel_2d[grid](
        mixed_qkv,
        z,
        b,
        a,
