...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    mock_cache_offset = mock_base + tl.arange(0, K_CACHE_STRIDE)
...
def trtllm_prefill_attn_kvfp8_dequant(
    ...
    assert block_tables_prefill.dtype == torch.int32, "block_tables_prefill must be int32 for int32 offset computation"
    assert kv_cache.numel() < 2 ** 31, "kv_cache total elements must be < 2^31"
    assert batch_size * num_of_page_per_token + 1 < 2 ** 31, "batch*page index must be < 2^31"
    assert (batch_size * num_of_page_per_token + 1) * 2 * k_cache_stride < 2 ** 31, "mock_kv_cache total elements must be < 2^31"
