...
def fn_triton_kernel(
    ...
    pid = tl.program_id(0)
    pid_head = pid % NUM_HEAD_TILES
    pid_token = pid // NUM_HEAD_TILES
...
def fn_triton(k, k_nope, k_rope, qk_nope_head_dim, qk_rope_head_dim, num_local_heads):
    ...
    BLOCK_ROWS = 16
    BLOCK_HEADS = 16
    num_head_tiles = triton.cdiv(num_local_heads, BLOCK_HEADS)
    num_token_tiles = triton.cdiv(num_tokens, BLOCK_ROWS)
    grid = (num_token_tiles * num_head_tiles,)
