...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    page_group_idx = tl.program_id(0).to(tl.int64)
    batch_idx = tl.program_id(1).to(tl.int64)
    block_table_stride_i64 = block_table_stride.to(tl.int64)
    dequant_dtype = mock_kv_cache_ptr.dtype.element_ty
    offsets = tl.arange(0, K_CACHE_STRIDE)
    if PAGES_PER_PROG == 1:
        logical_row = batch_idx * block_table_stride_i64 + page_group_idx
        orig_page_num = tl.load(block_tables_prefill_ptr + logical_row).to(tl.int64)
        if orig_page_num <= 0:
            return
        mock_base = (logical_row + 1) * KV_CACHE_STRIDE
        ...
        mock_cache_offset = mock_base + offsets
        ...
        mock_cache_offset = mock_base + K_CACHE_STRIDE + offsets
        dequantized_vals = dequantized_vals.to(dequant_dtype)
        tl.store(mock_kv_cache_ptr + mock_cache_offset, dequantized_vals)
    else:
        num_pages = block_table_stride_i64
        batch_base = batch_idx * block_table_stride_i64
        k_scale_val = tl.load(k_scale_ptr)
        v_scale_val = tl.load(v_scale_ptr)
        for i in range(PAGES_PER_PROG):
            ...
            if page_idx < num_pages:
                orig_page_num = tl.load(
                    block_tables_prefill_ptr + batch_base + page_idx
                ).to(tl.int64)
                if orig_page_num > 0:
                    mock_base = (batch_base + page_idx + 1) * KV_CACHE_STRIDE
                    offset = orig_page_num * KV_CACHE_STRIDE + offsets
                    fp8_vals = tl.load(kv_cache_ptr + offset)
                    dequantized_vals = fp8_vals.to(tl.float32) * k_scale_val
                    mock_cache_offset = mock_base + offsets
...
def trtllm_prefill_attn_kvfp8_dequant(
    ...
    if num_of_page_per_token > _GRID_CONSOLIDATE_THRESHOLD:
        ...
        grid = (triton.cdiv(num_of_page_per_token, pages_per_prog), batch_size)
    else:
        pages_per_prog = 1
        grid = (num_of_page_per_token, batch_size)
