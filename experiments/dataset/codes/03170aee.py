...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    col_idx = tl.arange(0, 2 * K_CACHE_STRIDE)
    scale_vec = tl.where(col_idx < K_CACHE_STRIDE, k_scale_val, v_scale_val)

    offset = orig_page_num * KV_CACHE_STRIDE + col_idx
    fp8_vals = tl.load(kv_cache_ptr + offset)
    dequantized_vals = fp8_vals.to(tl.float32) * scale_vec
    dequantized_vals = dequantized_vals.to(dequant_dtype)

    mock_cache_offset = (
        batch_idx * block_table_stride + mock_block_table_idx + 1
    ) * KV_CACHE_STRIDE + col_idx
