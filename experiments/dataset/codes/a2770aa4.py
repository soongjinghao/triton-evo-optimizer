...
def _selective_scan_update_kernel(
    ...
    B_ptr += pid_b * stride_B_batch + pg * stride_B_group
    C_ptr += pid_b * stride_C_batch + pg * stride_C_group
    if HAS_Z:
...
def selective_state_update(
    ...
    BLOCK_SIZE_M, num_warps = (
        ...
        if dstate <= 16
            ...
            (32, 8)
            if dstate <= 32
            else ((16, 8) if dstate <= 64 else ((8, 8) if dstate <= 128 else (4, 8)))
        )
    )
