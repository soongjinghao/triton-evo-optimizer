...
def fused_qkvzba_split_reshape_cat_kernel(
    ...
    if CONSOLIDATE:
        ...
        if i_batch < TOTAL_BATCH:
            for i_qk in range(NUM_HEADS_QK):
                base_ptr = mixed_qkvz + i_batch * NUM_HEADS_QK * QKVZ_DIM_T + i_qk * QKVZ_DIM_T
                ...
                blk_q_st_ptr = mixed_qkv + i_batch * NUM_HEADS_QK * QKV_DIM_T + i_qk * HEAD_QK
                blk_k_st_ptr = mixed_qkv + i_batch * NUM_HEADS_QK * QKV_DIM_T + NUM_HEADS_QK * HEAD_QK + i_qk * HEAD_QK
                blk_v_st_ptr = mixed_qkv + i_batch * NUM_HEADS_QK * QKV_DIM_T + NUM_HEADS_QK * HEAD_QK * 2 + i_qk * HEAD_V * NUM_HEADS_V_PER_QK
                ...
                blk_z_st_ptr = z + i_batch * NUM_HEADS_V * HEAD_V + i_qk * HEAD_V * NUM_HEADS_V_PER_QK
                tl.store(blk_z_st_ptr + tl.arange(0, HEAD_V * NUM_HEADS_V_PER_QK), z_vals)

                ba_base_ptr = mixed_ba + i_batch * NUM_HEADS_QK * BA_DIM_T + i_qk * BA_DIM_T
                b_vals = tl.load(ba_base_ptr + tl.arange(0, b_end))
                a_vals = tl.load(ba_base_ptr + tl.arange(b_end, a_end))
                blk_b_st_ptr = b + i_batch * NUM_HEADS_V + i_qk * NUM_HEADS_V_PER_QK
                blk_a_st_ptr = a + i_batch * NUM_HEADS_V + i_qk * NUM_HEADS_V_PER_QK
...
def fused_qkvzba_split_reshape_cat(
    ...
    use_consolidated = num_heads_qk <= 16
    if use_consolidated:
        grid = (TOTAL_BATCH,)
    else:
