...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    # P2 page-contiguous K/V combined load/store path.
    # Enabled only when KV_CACHE_STRIDE is exactly 2 * K_CACHE_STRIDE,
    # which is guaranteed by the wrapper layout s[1] == 2.
    if KV_CACHE_STRIDE == 2 * K_CACHE_STRIDE:
        cache_offsets = orig_page_num * KV_CACHE_STRIDE + tl.arange(0, KV_CACHE_STRIDE)
        fp8_vals_kv = tl.load(kv_cache_ptr + cache_offsets)

        scale_vec = tl.where(
            tl.arange(0, KV_CACHE_STRIDE) < K_CACHE_STRIDE,
            k_scale_val,
            v_scale_val,
        )
        dequantized_kv = fp8_vals_kv.to(tl.float32) * scale_vec

        mock_cache_offsets = (
            (batch_idx * block_table_stride + mock_block_table_idx + 1) * KV_CACHE_STRIDE
            + tl.arange(0, KV_CACHE_STRIDE)
        )
        tl.store(mock_kv_cache_ptr + mock_cache_offsets, dequantized_kv.to(dequant_dtype))
    else:
        # Fallback original K/V two-pass path.
        offset_k = orig_page_num * KV_CACHE_STRIDE + tl.arange(0, K_CACHE_STRIDE)
        fp8_vals_k = tl.load(kv_cache_ptr + offset_k)
        ...
        tl.store(mock_kv_cache_ptr + mock_cache_offset_k, dequantized_k.to(dequant_dtype))

        offset_v = orig_page_num * KV_CACHE_STRIDE + K_CACHE_STRIDE + tl.arange(0, K_CACHE_STRIDE)
        fp8_vals_v = tl.load(kv_cache_ptr + offset_v)
        ...
        tl.store(mock_kv_cache_ptr + mock_cache_offset_v, dequantized_v.to(dequant_dtype))
