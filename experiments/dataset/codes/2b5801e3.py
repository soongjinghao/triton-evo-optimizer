...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    tl.store(mock_kv_cache_ptr + mock_base + offs, dequantized_vals.to(dequant_dtype))

    v_scale_val = tl.load(v_scale_ptr)
    fp8_vals = tl.load(kv_cache_ptr + orig_base + K_CACHE_STRIDE + offs)
    dequantized_vals = fp8_vals.to(tl.float32) * v_scale_val
    tl.store(mock_kv_cache_ptr + mock_base + K_CACHE_STRIDE + offs, dequantized_vals.to(dequant_dtype))
