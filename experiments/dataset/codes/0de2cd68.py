...
def reshape_and_cache_flash_kernel(
    ...
    chunk_offset = chunk_idx * CHUNK_ELEMENTS
    chunk_mask = (chunk_offset + chunk_i) < n

    k = tl.load(key_base + chunk_offset + chunk_i, mask=chunk_mask)
    v = tl.load(value_base + chunk_offset + chunk_i, mask=chunk_mask)

    cache_offset = base_cache_offset + chunk_offset
