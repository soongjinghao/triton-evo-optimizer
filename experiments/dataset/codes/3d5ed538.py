...
def _trtllm_prefill_attn_kvfp8_dequant_compact(
    ...
    dequantized_k = fp8_vals_k.to(tl.float32) * k_scale_val
    mock_cache_offset_k = (
        batch_idx * block_table_stride + mock_idx + 1
    ) * KV_CACHE_STRIDE + tl.arange(0, K_CACHE_STRIDE)
    tl.store(tl.multiple_of(mock_kv_cache_ptr + mock_cache_offset_k, 16), dequantized_k.to(dequant_dtype))
    ...
    dequantized_v = fp8_vals_v.to(tl.float32) * v_scale_val
    mock_cache_offset_v = (
        (batch_idx * block_table_stride + mock_idx + 1) * KV_CACHE_STRIDE
        + K_CACHE_STRIDE
        + tl.arange(0, K_CACHE_STRIDE)
    )
    tl.store(tl.multiple_of(mock_kv_cache_ptr + mock_cache_offset_v, 16), dequantized_v.to(dequant_dtype))
...
def trtllm_prefill_attn_kvfp8_dequant(
    ...
    if num_valid > 0:
        valid_indices = valid_indices.contiguous()
        batch_indices = valid_indices[:, 0].contiguous()
        mock_indices = valid_indices[:, 1].contiguous()
        orig_page_nums = block_tables_prefill[batch_indices, mock_indices].contiguous()
