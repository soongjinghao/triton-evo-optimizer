...
def fused_qkvzba_split_reshape_cat_kernel(
    ...
    TOTAL_HEAD_BLOCKS: tl.constexpr = (NUM_HEADS_QK + BLOCK_H - 1) // BLOCK_H
    ...
    if ((TOTAL_BATCH % BLOCK_B) == 0) and ((NUM_HEADS_QK % BLOCK_H) == 0):
        for h_offset in range(BLOCK_H):
            i_qk = i_head_block * BLOCK_H + h_offset
...
def fused_qkvzba_split_reshape_cat(
    ...
    BLOCK_B = 4 if TOTAL_BATCH % 4 == 0 else 2
    BLOCK_H = 2 if num_heads_qk % 2 == 0 else 1

    grid = (
        triton.cdiv(TOTAL_BATCH, BLOCK_B) * triton.cdiv(num_heads_qk, BLOCK_H),
    )
