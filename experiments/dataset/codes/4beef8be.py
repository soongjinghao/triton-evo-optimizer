...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    arange0 = tl.arange(0, K_CACHE_STRIDE)
    ...
    fp8_vals = tl.load(kv_cache_ptr + orig_base + arange0)
    ...
    fp8_vals = tl.load(kv_cache_ptr + orig_base + K_CACHE_STRIDE + arange0)
    dequantized_vals = fp8_vals.to(tl.float32) * v_scale_val
    dequantized_vals = dequantized_vals.to(dequant_dtype)
    tl.store(
        mock_kv_cache_ptr + mock_base + K_CACHE_STRIDE + arange0,
        dequantized_vals,
    )
