...
def reduce_segments(
    ...
    seg_off = tl.arange(0, NUM_SEGMENTS_PER_SEQ)
    head_off = tl.arange(0, HEAD_SIZE_PADDED)
    segm_mask = seg_off < tl.full(
        [NUM_SEGMENTS_PER_SEQ], act_num_segments, dtype=tl.int32
    )
    dim_mask = head_off < HEAD_SIZE
    segm_offset = (
    ...
    segm_exp_scale = tl.exp(segm_max - overall_max)
    segm_expsum = segm_expsum * segm_exp_scale
    overall_expsum = tl.sum(segm_expsum)
    segm_output_offset = (
        query_token_idx.to(tl.int64)
        * (num_query_heads * NUM_SEGMENTS_PER_SEQ * HEAD_SIZE_PADDED)
        + query_head_idx * (NUM_SEGMENTS_PER_SEQ * HEAD_SIZE_PADDED)
        + seg_off[:, None] * HEAD_SIZE_PADDED
        + head_off[None, :]
    )
    ...
    segm_output *= segm_exp_scale[:, None]
