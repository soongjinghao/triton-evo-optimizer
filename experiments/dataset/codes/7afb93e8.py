...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    block_table_offset = batch_idx * block_table_stride + mock_block_table_idx
    orig_page_num = tl.load(block_tables_prefill_ptr + block_table_offset).to(tl.int64)
    if orig_page_num <= 0:
        return
    ...
    mock_page_num = block_table_offset + 1
    mock_base = mock_page_num * KV_CACHE_STRIDE
    orig_k_ptr = kv_cache_ptr + orig_base
    mock_k_ptr = mock_kv_cache_ptr + mock_base
    fp8_vals = tl.load(orig_k_ptr + offs)
    dequantized_vals = (fp8_vals.to(tl.float32) * k_scale_val).to(dequant_dtype)
    tl.store(mock_k_ptr + offs, dequantized_vals)
    v_scale_val = tl.load(v_scale_ptr)
    fp8_vals = tl.load(orig_k_ptr + K_CACHE_STRIDE + offs)
    dequantized_vals = (fp8_vals.to(tl.float32) * v_scale_val).to(dequant_dtype)
    tl.store(mock_k_ptr + K_CACHE_STRIDE + offs, dequantized_vals)
