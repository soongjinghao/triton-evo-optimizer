...
@triton.jit
def _recompute_w_u_fwd_kernel_loop(
    k,
    v,
    beta,
    w,
...
@triton.jit
def _recompute_w_u_fwd_kernel_expand(
    k,
    v,
    beta,
    w,
...
def recompute_w_u_fwd(
    ...
    num_v = triton.cdiv(V, BV)
    num_k = triton.cdiv(K, BK)

    log2_num_k = None
    if num_k & (num_k - 1) == 0:
        log2_num_k = num_k.bit_length() - 1

    expand = log2_num_k is not None and (num_v > 1 or num_k > 1)

    u = torch.empty_like(v)
    w = k.new_empty(B, T, H, K)

    if expand:
        grid = (NT, B * H, num_v * num_k)
        _recompute_w_u_fwd_kernel_expand[grid](
            k=k,
            v=v,
            beta=beta,
            w=w,
            ...
            NUM_K=num_k,
            NUM_V=num_v,
            LOG2_NUM_K=log2_num_k,
        )
    else:
        grid = (NT, B * H)
        _recompute_w_u_fwd_kernel_loop[grid](
            k=k,
            v=v,
            beta=beta,
            w=w,
