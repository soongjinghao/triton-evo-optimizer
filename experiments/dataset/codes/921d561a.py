...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    arange0 = tl.arange(0, 2 * K_CACHE_STRIDE)
    src_offset = orig_page_num * KV_CACHE_STRIDE + arange0
    fp8_vals = tl.load(kv_cache_ptr + src_offset)

    fp8_vals_2d = tl.view(fp8_vals, (2, K_CACHE_STRIDE))
    row_ids = tl.arange(0, 2)[:, None]
    scales = tl.where(row_ids == 0, k_scale_val, v_scale_val)
    dequantized_2d = fp8_vals_2d.to(tl.float32) * scales
    dequantized_vals = tl.view(dequantized_2d, (2 * K_CACHE_STRIDE,))
    dequantized_vals = dequantized_vals.to(dequant_dtype)

    mock_base = (
        batch_idx * block_table_stride + mock_block_table_idx + 1
    ) * KV_CACHE_STRIDE
    tl.store(mock_kv_cache_ptr + mock_base + arange0, dequantized_vals)
