...
def merge_16x16_to_32x32_inverse_kernel(
    ...
    if IS_VARLEN:
        ...
        start_chunk = i_pack * PACK_K
        valid_count = tl.minimum(PACK_K, num_chunks - start_chunk)
        bos = i_b * T
        eos = i_b * T + T
    ...
    for k in tl.static_range(PACK_K):
        if k < valid_count:
            ...
            limit1 = tl.minimum(16, T - i_t_bt)
            base_A = A + i_t_bt * H * BT
            for i in range(2, limit1):
                b_a_11 = -tl.load(base_A + i * H * BT + o_i)
                b_a_11 += tl.sum(b_a_11[:, None] * b_Ai_11, 0)
                b_Ai_11 = tl.where((o_i == i)[:, None], b_a_11, b_Ai_11)

            limit2 = tl.minimum(32, T - i_t_bt)
            for i in range(18, limit2):
                b_a_22 = -tl.load(base_A + i * H * BT + o_i + 16)
