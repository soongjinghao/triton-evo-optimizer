...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    arange_cache = tl.arange(0, K_CACHE_STRIDE)
    mock_row_idx = batch_idx * block_table_stride + mock_block_table_idx + 1

    offset_k = orig_page_num * KV_CACHE_STRIDE + arange_cache
    fp8_vals_k = tl.load(tl.multiple_of(kv_cache_ptr + offset_k, 16))
    dequantized_k = fp8_vals_k.to(tl.float32) * k_scale_val
    mock_cache_offset_k = mock_row_idx * KV_CACHE_STRIDE + arange_cache
    tl.store(tl.multiple_of(mock_kv_cache_ptr + mock_cache_offset_k, 16), dequantized_k.to(dequant_dtype))

    offset_v = orig_page_num * KV_CACHE_STRIDE + K_CACHE_STRIDE + arange_cache
    fp8_vals_v = tl.load(tl.multiple_of(kv_cache_ptr + offset_v, 16))
    dequantized_v = fp8_vals_v.to(tl.float32) * v_scale_val
    mock_cache_offset_v = mock_row_idx * KV_CACHE_STRIDE + K_CACHE_STRIDE + arange_cache
...
def _trtllm_prefill_attn_kvfp8_dequant_int32(
    ...
    orig_page_num = tl.load(
        block_tables_prefill_ptr + (batch_idx * block_table_stride + mock_block_table_idx).to(tl.int64)
    )
    if orig_page_num <= 0:
        return
    dequant_dtype = mock_kv_cache_ptr.dtype.element_ty
    k_scale_val = tl.load(k_scale_ptr)
    v_scale_val = tl.load(v_scale_ptr)
    orig_page_num_i64 = orig_page_num.to(tl.int64)
    arange_k = tl.arange(0, K_CACHE_STRIDE).to(tl.int64)
    mock_row_idx = (batch_idx * block_table_stride + mock_block_table_idx + 1).to(tl.int64)

    offset_k = orig_page_num_i64 * KV_CACHE_STRIDE + arange_k
    fp8_vals_k = tl.load(tl.multiple_of(kv_cache_ptr + offset_k, 16))
    dequantized_k = fp8_vals_k.to(tl.float32) * k_scale_val
    mock_cache_offset_k = mock_row_idx * KV_CACHE_STRIDE + arange_k
    tl.store(tl.multiple_of(mock_kv_cache_ptr + mock_cache_offset_k, 16), dequantized_k.to(dequant_dtype))

    offset_v = orig_page_num_i64 * KV_CACHE_STRIDE + K_CACHE_STRIDE + arange_k
    fp8_vals_v = tl.load(tl.multiple_of(kv_cache_ptr + offset_v, 16))
    dequantized_v = fp8_vals_v.to(tl.float32) * v_scale_val
    mock_cache_offset_v = mock_row_idx * KV_CACHE_STRIDE + K_CACHE_STRIDE + arange_k
    tl.store(tl.multiple_of(mock_kv_cache_ptr + mock_cache_offset_v, 16), dequantized_v.to(dequant_dtype))
def trtllm_prefill_attn_kvfp8_dequant(
    ...
    if (
        batch_size * num_of_page_per_token < 2**31
        and kv_cache_stride < 2**31
        and s[0] * kv_cache_stride < 2**31
    ):
