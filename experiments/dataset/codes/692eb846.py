...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    offsets = tl.arange(0, K_CACHE_STRIDE)
    if PAGES_PER_PROG == 1:
        orig_page_num = tl.load(block_tables_prefill_ptr + batch_idx * block_table_stride + page_group_idx).to(tl.int64)
        if orig_page_num <= 0:
            return
        k_scale_val = tl.load(k_scale_ptr)
        offset = orig_page_num * KV_CACHE_STRIDE + offsets
        fp8_vals = tl.load(kv_cache_ptr + offset)
        dequantized_vals = fp8_vals.to(tl.float32) * k_scale_val
        mock_cache_offset = (batch_idx * block_table_stride + page_group_idx + 1) * KV_CACHE_STRIDE + offsets
        dequantized_vals = dequantized_vals.to(dequant_dtype)
        tl.store(mock_kv_cache_ptr + mock_cache_offset, dequantized_vals)
        v_scale_val = tl.load(v_scale_ptr)
        offset = orig_page_num * KV_CACHE_STRIDE + K_CACHE_STRIDE + offsets
        fp8_vals = tl.load(kv_cache_ptr + offset)
        dequantized_vals = fp8_vals.to(tl.float32) * v_scale_val
        mock_cache_offset = (batch_idx * block_table_stride + page_group_idx + 1) * KV_CACHE_STRIDE + K_CACHE_STRIDE + offsets
        dequantized_vals = dequantized_vals.to(dequant_dtype)
        tl.store(mock_kv_cache_ptr + mock_cache_offset, dequantized_vals)
    else:
        num_pages = block_table_stride
        k_scale_val = tl.load(k_scale_ptr)
        v_scale_val = tl.load(v_scale_ptr)
        for i in range(PAGES_PER_PROG):
            page_idx = page_group_idx * PAGES_PER_PROG + i
            if page_idx < num_pages:
                orig_page_num = tl.load(block_tables_prefill_ptr + batch_idx * block_table_stride + page_idx).to(tl.int64)
                if orig_page_num > 0:
                    offset = orig_page_num * KV_CACHE_STRIDE + offsets
                    fp8_vals = tl.load(kv_cache_ptr + offset)
                    dequantized_vals = fp8_vals.to(tl.float32) * k_scale_val
                    mock_cache_offset = (batch_idx * block_table_stride + page_idx + 1) * KV_CACHE_STRIDE + offsets
                    dequantized_vals = dequantized_vals.to(dequant_dtype)
                    tl.store(mock_kv_cache_ptr + mock_cache_offset, dequantized_vals)
                    offset = orig_page_num * KV_CACHE_STRIDE + K_CACHE_STRIDE + offsets
                    fp8_vals = tl.load(kv_cache_ptr + offset)
                    dequantized_vals = fp8_vals.to(tl.float32) * v_scale_val
                    mock_cache_offset = (batch_idx * block_table_stride + page_idx + 1) * KV_CACHE_STRIDE + K_CACHE_STRIDE + offsets
                    dequantized_vals = dequantized_vals.to(dequant_dtype)
                    tl.store(mock_kv_cache_ptr + mock_cache_offset, dequantized_vals)
...
def trtllm_prefill_attn_kvfp8_dequant(
    ...
    if num_of_page_per_token > _GRID_CONSOLIDATE_THRESHOLD:
        pages_per_prog = _PAGES_PER_PROG
        grid = (batch_size, triton.cdiv(num_of_page_per_token, pages_per_prog))
    else:
        pages_per_prog = 1
