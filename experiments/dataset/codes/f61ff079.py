...
def reshape_and_cache_flash_kernel(
    ...
    k_cache_base = key_cache + base_cache_offset
    v_cache_base = value_cache + base_cache_offset
    ...
    for chunk_idx in range(0, num_chunks):
        ...
        k_cache_ptr = k_cache_base + offset
        v_cache_ptr = v_cache_base + offset
        k = tl.load(key_ptr + chunk_i, mask=chunk_mask)
        v = tl.load(value_ptr + chunk_i, mask=chunk_mask)
        tl.store(k_cache_ptr + chunk_i, k, mask=chunk_mask)
        tl.store(v_cache_ptr + chunk_i, v, mask=chunk_mask)
