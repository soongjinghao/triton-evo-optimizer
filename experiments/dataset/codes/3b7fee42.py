...
def fn_triton_kernel(
    ...
    vals_rope_b = tl.broadcast_to(vals_rope, (BLOCK_ROWS, BLOCK_HEADS, QK_ROPE_HEAD_DIM))

    full_sub_id = tl.arange(0, QK_NOPE_HEAD_DIM + QK_ROPE_HEAD_DIM)
    offs_k_full = (
        token_id[:, None, None] * K_STRIDE_0
        + head_offs_k[None, :, None]
        + full_sub_id[None, None, :]
    )

    cat_vals = tl.cat(vals_nope, vals_rope_b, dim=2)
    tl.store(k_ptr + offs_k_full, cat_vals, mask=mask)
