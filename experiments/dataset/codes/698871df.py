...
def merge_16x16_to_32x32_inverse_kernel(
    ...
    if T_rem >= 32:
        # Full 32-row block path: both 16x16 subtiles are provably in-bounds.
        # Use static loop bounds and remove boundary_check from block_ptr
        # access only in this branch.
        if not USE_TMA:
            ...
            b_Ai_11 = tl.load(p_A_11).to(tl.float32)
            b_Ai_22 = tl.load(p_A_22).to(tl.float32)
        else:
        ...
        if not USE_TMA:
            ...
            b_A_21 = tl.load(p_A_21).to(tl.float32)
        else:
        ...
        # Tail path: preserve original dynamic limits and boundary checks.
        # Never overfetch a full 32x32 block when T_rem < 32.
        if not USE_TMA:
            p_A_11 = tl.make_block_ptr(
                A, (T, BT), (H * BT, 1), (i_t * BT, 0), (16, 16), (1, 0)
            )
