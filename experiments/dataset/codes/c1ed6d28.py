...
def merge_16x16_to_32x32_inverse_kernel(
    ...
    if IS_VARLEN:
        seq_idx = tl.load(pack_meta + i_chunk * 3).to(tl.int32)
        start_chunk = tl.load(pack_meta + i_chunk * 3 + 1).to(tl.int32)
        valid_count = tl.load(pack_meta + i_chunk * 3 + 2).to(tl.int32)
...
def solve_tril(
    ...
    if cu_seqlens is not None:
        ...
        for seq_idx, length in enumerate(lengths):
            ...
            for c in range(cnt):
                pack_meta_rows.append((seq_idx, c, 1))
        if len(pack_meta_rows) == 0:
            return torch.zeros_like(A, dtype=output_dtype)
