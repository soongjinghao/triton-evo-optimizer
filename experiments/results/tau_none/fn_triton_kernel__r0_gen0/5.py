...
def fn_triton(k, k_nope, k_rope, qk_nope_head_dim, qk_rope_head_dim, num_local_heads):
    ...
    # Token 轴打包:num_tokens 足够大且 tile 面积未超 UB 预算时,
    # 将 BLOCK_ROWS 由 16 提升到 32,减少尾部 block 比例与 PID 数量。
    block_heads = 16
    max_dim = max(qk_nope_head_dim, qk_rope_head_dim)
    tile_elems = 32 * block_heads * max_dim
    ub_tile_limit = 65536

    block_rows = 16
    if num_tokens >= 256 and tile_elems <= ub_tile_limit:
        block_rows = 32
