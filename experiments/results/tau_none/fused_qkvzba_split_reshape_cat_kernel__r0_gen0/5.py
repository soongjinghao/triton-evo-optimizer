...
def fused_qkvzba_split_reshape_cat_kernel(
    ...
    row_stride_qkvz = NUM_HEADS_QK * QKVZ_DIM_T
    i_qk_qkvz_offset = i_qk * QKVZ_DIM_T
    row_stride_qkv = NUM_HEADS_QK * QKV_DIM_T
    i_qk_q_offset = i_qk * HEAD_QK
    i_qk_k_offset = NUM_HEADS_QK * HEAD_QK + i_qk * HEAD_QK
    i_qk_v_offset = NUM_HEADS_QK * HEAD_QK * 2 + i_qk * HEAD_V * NUM_HEADS_V_PER_QK
    row_stride_z = NUM_HEADS_V * HEAD_V
    i_qk_z_offset = i_qk * HEAD_V * NUM_HEADS_V_PER_QK
    row_stride_ba = NUM_HEADS_QK * BA_DIM_T
    i_qk_ba_offset = i_qk * BA_DIM_T
    row_stride_b = NUM_HEADS_V
    i_qk_b_offset = i_qk * NUM_HEADS_V_PER_QK
    for b_offset in range(BLOCK_B):
        b_idx = i_bs_block * BLOCK_B + b_offset
        if b_idx < TOTAL_BATCH:
            base_ptr = mixed_qkvz + b_idx * row_stride_qkvz + i_qk_qkvz_offset
            ...
            blk_q_st_ptr = mixed_qkv + b_idx * row_stride_qkv + i_qk_q_offset
            blk_k_st_ptr = mixed_qkv + b_idx * row_stride_qkv + i_qk_k_offset
            blk_v_st_ptr = mixed_qkv + b_idx * row_stride_qkv + i_qk_v_offset
            ...
            blk_z_st_ptr = z + b_idx * row_stride_z + i_qk_z_offset
            tl.store(blk_z_st_ptr + tl.arange(0, HEAD_V * NUM_HEADS_V_PER_QK), z_vals)
            ba_base_ptr = mixed_ba + b_idx * row_stride_ba + i_qk_ba_offset
            ...
            blk_b_st_ptr = b + b_idx * row_stride_b + i_qk_b_offset
            blk_a_st_ptr = a + b_idx * row_stride_b + i_qk_b_offset
