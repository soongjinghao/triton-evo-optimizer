...
def _get_autotuned_kernel(K: int, V: int):
    configs = []
    for num_warps in [2, 4, 8]:
        for num_stages in [2, 3, 4]:
            configs.append(triton.Config({"BK": 64, "BV": 64}, num_warps=num_warps, num_stages=num_stages))
            if K >= 128 and V >= 128:
                configs.append(triton.Config({"BK": 128, "BV": 128}, num_warps=num_warps, num_stages=num_stages))
    kernel = triton.autotune(configs=configs, key=["H", "K", "V", "BT", "BK", "BV", "IS_VARLEN"])(recompute_w_u_fwd_kernel)
    kernel = triton.heuristics({"IS_VARLEN": lambda args: args["cu_seqlens"] is not None})(kernel)
    return kernel
...
def recompute_w_u_fwd(
    ...
    kernel = _get_autotuned_kernel(K, V)
    kernel[(NT, B * H)](
        k=k,
        v=v,
        beta=beta,
