...
def _selective_scan_update_kernel(
    ...
    pb = tl.program_id(axis=1)
    ph = tl.program_id(axis=2)
    pg = ph // nheads_ngroups_ratio

    if HAS_STATE_BATCH_INDICES:
        state_batch_indices_ptr += pb
        state_batch_idx = tl.load(state_batch_indices_ptr).to(tl.int64)
        state_ptr += state_batch_idx * stride_state_batch + ph * stride_state_head
    else:
        state_ptr += pb * stride_state_batch + ph * stride_state_head

    x_ptr += pb * stride_x_batch + ph * stride_x_head
    dt_ptr += pb * stride_dt_batch + ph * stride_dt_head
    if HAS_DT_BIAS:
        dt_bias_ptr += ph * stride_dt_bias_head
    A_ptr += ph * stride_A_head
    B_ptr += pb * stride_B_batch + pg * stride_B_group
    C_ptr += pb * stride_C_batch + pg * stride_C_group
    if HAS_D:
        D_ptr += ph * stride_D_head
    if HAS_Z:
        z_ptr += pb * stride_z_batch + ph * stride_z_head
    out_ptr += pb * stride_out_batch + ph * stride_out_head
    ...
    if HAS_STATE_BATCH_INDICES:
        mask_state = mask_2d & (state_batch_idx != pad_slot_id)
    else:
        mask_state = mask_2d
    state = tl.load(state_ptrs, mask=mask_state, other=0.0)
