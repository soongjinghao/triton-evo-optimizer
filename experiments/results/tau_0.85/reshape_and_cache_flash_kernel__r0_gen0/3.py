...
def reshape_and_cache_flash_kernel(
    ...
    for chunk_idx in range(0, num_chunks):
        chunk_mask = chunk_i < rem

        k = tl.load(key_base + chunk_i, mask=chunk_mask)
        v = tl.load(value_base + chunk_i, mask=chunk_mask)

        tl.store(key_cache_base + chunk_i, k, mask=chunk_mask)
        tl.store(value_cache_base + chunk_i, v, mask=chunk_mask)

        key_base = key_base + CHUNK_ELEMENTS
        value_base = value_base + CHUNK_ELEMENTS
        key_cache_base = key_cache_base + CHUNK_ELEMENTS
        value_cache_base = value_cache_base + CHUNK_ELEMENTS
        rem = rem - CHUNK_ELEMENTS
