...
def fused_qkvzba_split_reshape_cat_kernel(
    ...
    for b_offset in range(BLOCK_B):
        ...
        b_idx = b_idx + 1
        base_ptr = base_ptr + NUM_HEADS_QK * QKVZ_DIM_T
        blk_q_st_ptr = blk_q_st_ptr + NUM_HEADS_QK * QKV_DIM_T
        blk_k_st_ptr = blk_k_st_ptr + NUM_HEADS_QK * QKV_DIM_T
        blk_v_st_ptr = blk_v_st_ptr + NUM_HEADS_QK * QKV_DIM_T
        blk_z_st_ptr = blk_z_st_ptr + NUM_HEADS_V * HEAD_V
        ba_base_ptr = ba_base_ptr + NUM_HEADS_QK * BA_DIM_T
        blk_b_st_ptr = blk_b_st_ptr + NUM_HEADS_V
        blk_a_st_ptr = blk_a_st_ptr + NUM_HEADS_V
