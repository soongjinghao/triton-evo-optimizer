...
def merge_16x16_to_32x32_inverse_kernel(
    ...
    if H & (H - 1) == 0 and BT == 32:
        LOG2_H = int(math.log2(H.value))
        i_b = i_bh >> LOG2_H
        i_h = i_bh & (H - 1)
    else:
        i_b = i_bh // H
    ...
    if H & (H - 1) == 0 and BT == 32:
        off = row_base + 2 * stride_hbt
        for i in range(2, limit1):
            b_a_11 = -tl.load(A + off + o_i)
    ...
    if H & (H - 1) == 0 and BT == 32:
        off = row_base + 18 * stride_hbt
        for i in range(18, limit2):
            b_a_22 = -tl.load(A + off + o_i + 16)
