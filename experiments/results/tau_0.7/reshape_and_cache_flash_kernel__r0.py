    ...
    for j in range(BLOCK_TOKENS):
        ...
        if token_idx < num_tokens:
            ...
            if slot_idx >= 0:
                ...
                key_token_base = key + token_idx * key_stride
                value_token_base = value + token_idx * value_stride
                key_cache_base = key_cache + base_cache_offset
                value_cache_base = value_cache + base_cache_offset
                if n <= CHUNK_ELEMENTS:
                    key_ptr = key_token_base + offs
                    value_ptr = value_token_base + offs
                    k = tl.load(key_ptr, mask=mask)
                    v = tl.load(value_ptr, mask=mask)
                    tl.store(key_cache_base + offs, k, mask=mask)
                    tl.store(value_cache_base + offs, v, mask=mask)
                else:
                    for chunk_idx in range(0, num_chunks):
                        offset = chunk_idx * CHUNK_ELEMENTS
                        chunk_mask = (offset + chunk_i) < n
                        key_ptr = key_token_base + offset
                        value_ptr = value_token_base + offset
