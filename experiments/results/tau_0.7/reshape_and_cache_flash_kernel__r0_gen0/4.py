...
def reshape_and_cache_flash_kernel(
    ...
    if n <= FULL_N_MAX:
        ...
        key_ptr = key + token_idx * key_stride + offs
        value_ptr = value + token_idx * value_stride + offs
        k = tl.load(key_ptr, mask=mask)
        v = tl.load(value_ptr, mask=mask)

        cache_offset = base_cache_offset + offs
        tl.store(key_cache + cache_offset, k, mask=mask)
        tl.store(value_cache + cache_offset, v, mask=mask)
    else:
