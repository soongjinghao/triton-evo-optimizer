...
def recompute_w_u_fwd_kernel(
    ...
    for c in range(CHUNKS_PER_PROG):
        i_t = pid0 * CHUNKS_PER_PROG + c
        if i_t < NT:
            if IS_VARLEN:
...
def recompute_w_u_fwd(
    ...
    CHUNKS_PER_PROG = 2
    grid = (triton.cdiv(NT, CHUNKS_PER_PROG), B * H)
    BK = 64
    BV = 64
    ...
    recompute_w_u_fwd_kernel[grid](
        ...
        BK=BK,
        BV=BV,
        CHUNKS_PER_PROG=CHUNKS_PER_PROG,
    )
    return w, u