...
def recompute_w_u_fwd_kernel(
    ...
    i_t, i_bg = tl.program_id(0), tl.program_id(1)
    i_b = i_bg // Hg
    i_g = i_bg % Hg

    H_PER_G: tl.constexpr = H // Hg
    ...
    # V 维:每个 head 独立加载、计算并写回 u
    for i_hl in range(H_PER_G):
        i_h = i_g * H_PER_G + i_hl
    ...
    # K 维:同一 head group 共享 K 块加载,再分别计算各 head 的 w
...
def recompute_w_u_fwd(
    ...
    recompute_w_u_fwd_kernel[(NT, B * Hg)](
        k=k,
        v=v,
        beta=beta,
        w=w,
