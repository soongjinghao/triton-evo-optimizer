...
def fn_triton(k, k_nope, k_rope, qk_nope_head_dim, qk_rope_head_dim, num_local_heads):
    ...
    # Keep total tile area constant at 256, but choose an asymmetric row/head
    # ratio according to the input shape to improve MTE3 write efficiency.
    if num_tokens >= num_local_heads:
        block_rows = 32
        block_heads = 8
    else:
        block_rows = 8
