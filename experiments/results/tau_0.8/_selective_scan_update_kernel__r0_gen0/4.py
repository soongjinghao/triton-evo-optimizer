...
@triton.heuristics(
    {"STRIDE_STATE_DSTATE_IS_ONE": lambda args: args["stride_state_dstate"] == 1}
)
@triton.heuristics(
    {"STRIDE_A_DSTATE_IS_ONE": lambda args: args["stride_A_dstate"] == 1}
)
@triton.jit
def _selective_scan_update_kernel(
    ...
    STRIDE_STATE_DSTATE_IS_ONE: tl.constexpr,
    STRIDE_A_DSTATE_IS_ONE: tl.constexpr,
    ...
    if STRIDE_A_DSTATE_IS_ONE:
        A_ptrs = A_ptr + (
            offs_m[:, None] * stride_A_dim + offs_n[None, :]
        )
    else:
        A_ptrs = A_ptr + (
