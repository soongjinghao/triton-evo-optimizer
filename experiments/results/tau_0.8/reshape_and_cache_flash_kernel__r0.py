...
def reshape_and_cache_flash_kernel(
    ...
    if head_size <= CHUNK_ELEMENTS and (CHUNK_ELEMENTS == (CHUNK_ELEMENTS // head_size) * head_size):
        BH: tl.constexpr = CHUNK_ELEMENTS // head_size
        head_idx = tl.arange(0, BH)
        dim_idx = tl.arange(0, head_size)
        offsets = head_idx[:, None] * head_size + dim_idx[None, :]
        for chunk_idx in tl.static_range(num_chunks):
            ...
            k = tl.load(key_base + offset + offsets, mask=load_mask)
            v = tl.load(value_base + offset + offsets, mask=load_mask)
            tl.store(key_cache_base + offset + offsets, k, mask=load_mask)
            tl.store(value_cache_base + offset + offsets, v, mask=load_mask)
    else:
