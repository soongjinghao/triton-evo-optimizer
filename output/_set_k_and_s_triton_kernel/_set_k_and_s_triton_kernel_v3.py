...
def _set_k_and_s_triton_kernel(
    ...
    k = tl.load(tl.multiple_of(index_k_ptr + in_k_offsets, 16))
    k_scale = tl.load(tl.multiple_of(index_k_scale_ptr + token_id, 4))
    loc_page_index = loc >> 6
    loc_token_offset_in_page = loc & 63
    out_k_offsets = (loc << 7) + (loc_page_index << 8) + tl.arange(0, NUM_K_ELEMS_PER_TOKEN)
    out_s_offset = (loc_page_index << 11) + (loc_page_index << 6) + loc_token_offset_in_page + 2048
