...
# 自动调优配置，搜索 num_warps 和 num_stages 的最优组合
@triton.autotune(
    configs=[
    ...
    key=['num_tokens_to_write'],   # 若 token 数量固定，可去除该 key 以简化
)
@triton.jit
def _set_k_and_s_triton_kernel(
    ...
    index_k_scale_ptr,
    stride_k0,
    num_tokens_to_write,          # 仅用于 autotune 的 key，核内逻辑不依赖
...
def _set_k_and_s_triton(
    ...
    # 自动调优会根据 num_tokens_to_write 选择最优的 num_warps 和 num_stages
    _set_k_and_s_triton_kernel[(num_tokens_to_write,)](
        ...
        index_k_scale,
        index_k.stride(0),
        num_tokens_to_write,          # 仅用于 key，不影响逻辑
        PAGE_SIZE=page_size,
