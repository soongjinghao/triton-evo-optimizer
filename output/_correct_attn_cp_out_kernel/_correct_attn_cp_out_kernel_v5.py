...
@triton.jit
def _correct_attn_cp_out_kernel_offspring(
    outputs_ptr,
    new_output_ptr,
    lses_ptr,
    vlse_ptr,
    ...
    vlse_offsets = batch_idx * lses_stride_B + head_idx * lses_stride_H
    tl.store(vlse_ptr + vlse_offsets, lse)
...
def correct_attn_cp_out(outputs, lses, lse_idx):
    ...
    _correct_attn_cp_out_kernel_offspring[grid, num_warps=8](
        outputs,
        new_output,
        lses,
        vlse,
