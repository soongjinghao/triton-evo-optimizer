...
def merge_16x16_to_64x64_inverse_kernel(
    ...
    for i in range(1, 16):
        v1 = tl.load(row_base + i * stride + o).to(tl.float32)
        r1 = tl.sum((-v1)[:, None] * D1, 0) - v1
        v2 = tl.load(row_base + (16 + i) * stride + 16 + o).to(tl.float32)
        r2 = tl.sum((-v2)[:, None] * D2, 0) - v2
        v3 = tl.load(row_base + (32 + i) * stride + 32 + o).to(tl.float32)
        r3 = tl.sum((-v3)[:, None] * D3, 0) - v3
        v4 = tl.load(row_base + (48 + i) * stride + 48 + o).to(tl.float32)
        r4 = tl.sum((-v4)[:, None] * D4, 0) - v4
        m = (o == i)[:, None]
        D1 = tl.where(m, r1, D1)
        D2 = tl.where(m, r2, D2)
        D3 = tl.where(m, r3, D3)
