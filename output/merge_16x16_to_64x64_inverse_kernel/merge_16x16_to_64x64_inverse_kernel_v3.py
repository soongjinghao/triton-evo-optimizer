...
def merge_16x16_to_64x64_inverse_kernel(
    ...
    for i in range(1, 16):
        r1 = -tl.load(row_base + i * stride + o).to(tl.float32)
        r1 += tl.sum(r1[:, None] * D1, 0)
        r2 = -tl.load(row_base + (16 + i) * stride + 16 + o).to(tl.float32)
        r2 += tl.sum(r2[:, None] * D2, 0)
        r3 = -tl.load(row_base + (32 + i) * stride + 32 + o).to(tl.float32)
        r3 += tl.sum(r3[:, None] * D3, 0)
        r4 = -tl.load(row_base + (48 + i) * stride + 48 + o).to(tl.float32)
