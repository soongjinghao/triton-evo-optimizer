...
def fused_gdn_gating(
    ...
    # BLK_HEADS: 直接取 next_power_of_2(num_heads),下限 1,上限 128
    BLK_HEADS = min(triton.next_power_of_2(num_heads), 128)

    # BLK_BATCH: 直接取 next_power_of_2(batch),下限 1,上限 64
    BLK_BATCH = min(triton.next_power_of_2(batch), 64)

    # 保护:若 tile 内总元素过少,尝试扩大 BLK_HEADS 以保证单 block 有足够工作量
    if BLK_HEADS * BLK_BATCH < 64:
        # 扩大 BLK_HEADS,但不能超过 num_heads 的 2 的幂上限和 128
