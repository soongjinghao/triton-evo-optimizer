...
def _chunk_state_fwd_kernel(
    ...
    SWIZZLE_GROUP_SIZE: tl.constexpr = 4,
):
    ...
    # Swizzle the 1D program_id(0) into a 2D mapping to improve L2 cache locality
    pid = tl.program_id(axis=0)
    num_pid_m = tl.cdiv(hdim, BLOCK_SIZE_M)
    num_pid_n = tl.cdiv(dstate, BLOCK_SIZE_N)
    pid_m_orig = pid // num_pid_n
    pid_n_orig = pid % num_pid_n
    # Apply block swizzling within groups of SWIZZLE_GROUP_SIZE
    group_m = pid_m_orig // SWIZZLE_GROUP_SIZE
    group_n = pid_n_orig // SWIZZLE_GROUP_SIZE
    pid_m = group_m * SWIZZLE_GROUP_SIZE + (pid_n_orig % SWIZZLE_GROUP_SIZE)
    pid_n = group_n * SWIZZLE_GROUP_SIZE + (pid_m_orig % SWIZZLE_GROUP_SIZE)
    # Ensure we don't exceed bounds (remaining blocks are handled by the original mapping)
    pid_m = tl.where(pid_m < num_pid_m, pid_m, pid_m_orig)
    pid_n = tl.where(pid_n < num_pid_n, pid_n, pid_n_orig)
    ...
    for k in range(0, chunk_size_limit, BLOCK_SIZE_K):
        ...
        # Efficient precision management: scale B in float32, cast back to native dtype
        # Use pointer's element type for safety (x_ptr.dtype.element_ty)
        b_fused = (b.to(tl.float32) * scale[:, None]).to(x_ptrs.dtype.element_ty)
        acc = tl.dot(x, b_fused, acc=acc)
...
def _chunk_state_fwd(
    ...
    # Adaptive block sizes: next_power_of_2 capped at 128 and aligned to 16
    BLOCK_SIZE_M = min(128, triton.next_power_of_2(headdim))
    BLOCK_SIZE_N = min(128, triton.next_power_of_2(dstate))
    BLOCK_SIZE_K = min(128, triton.next_power_of_2(chunk_size))
    ...
    NUM_WARPS = 8  # More warps to hide latency on Ascend

    # Swizzle group size: use 4 for typical sizes; can be tuned
    SWIZZLE_GROUP_SIZE = 4
    ...
    _chunk_state_fwd_kernel[grid](
        ...
        *(
        ...
        NUM_WARPS=NUM_WARPS,
        SWIZZLE_GROUP_SIZE=SWIZZLE_GROUP_SIZE,
    )
    return states