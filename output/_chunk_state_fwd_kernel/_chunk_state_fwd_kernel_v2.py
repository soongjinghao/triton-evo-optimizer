...
def _chunk_state_fwd_kernel(
    ...
    dA_cs_last = tl.load(
        tl.multiple_of(dA_cumsum_ptr, 16) + (chunk_size - 1) * stride_dA_cs_csize
    ).to(tl.float32)
    if HAS_SEQ_IDX:
        seq_idx_last = tl.load(
            tl.multiple_of(seq_idx_ptr, 16)
    ...
    for k in range(chunk_size_limit):
        ...
        dA_cs_k = tl.load(tl.multiple_of(dA_cs_off, 16)).to(tl.float32)
        dt_k = tl.load(tl.multiple_of(dt_off, 16)).to(tl.float32)
        if not HAS_SEQ_IDX:
            scale = tl.exp(dA_cs_last - dA_cs_k) * dt_k
        else:
            seq_idx_k = tl.load(tl.multiple_of(seq_idx_off, 16))
            scale = tl.where(
    ...
    states_ptr = tl.multiple_of(states_ptr, 16) + (
