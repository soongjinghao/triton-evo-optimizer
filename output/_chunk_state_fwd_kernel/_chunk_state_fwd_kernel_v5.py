...
def _chunk_state_fwd_kernel(
    ...
    for k in range(0, chunk_size_limit, BLOCK_SIZE_K):
        # Load x and b in their native dtype (e.g., float16/bf16)
        x = tl.load(
            x_ptrs,
        ...
        # Scale b in float32, then cast back to native dtype for dot product
        b_fused = (b.to(tl.float32) * scale[:, None]).to(x_ptr.dtype.element_ty)
...
def _chunk_state_fwd(
    ...
    # Adaptive block size selection (next power of 2, capped at 128, aligned to 16)
    ...
    NUM_WARPS = 8  # Use more warps to hide latency
