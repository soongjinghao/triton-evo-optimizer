...
def _per_token_group_quant_8bit_colmajor_kernel(
    ...
    in_ptrs = tl.multiple_of(y_ptr, 16) + m_offsets[:, None] * y_row_stride + g_start + col_offsets[None, :]
    out_ptrs = tl.multiple_of(y_q_ptr, 16) + m_offsets[:, None] * y_q_row_stride + g_start + col_offsets[None, :]
    y_s_ptrs = tl.multiple_of(y_s_ptr, 16) + pid_g * y_s_col_stride + m_offsets
    if not REQUIRES_M_MASK and not REQUIRES_N_MASK:
        y = tl.load(in_ptrs)
        _absmax = tl.max(tl.abs(y), axis=1)
        _absmax_f32 = tl.maximum(_absmax.to(tl.float32), eps)
        y_s = _absmax_f32 * INV_BIT8_MAX
        if SCALE_UE8M0:
