...
def _per_token_group_quant_8bit_colmajor(
    ...
    """Fused per-token-group quantization with one-pass HBM access.
    
    - For groups <= BLOCK: load entire group, compute max, scale, quantize in registers.
    - For larger groups: tile into BLOCK-sized chunks, buffer in shared memory,
      compute max in one sweep, then quantize from shared memory (no second HBM read).
    """
    pid = tl.program_id(0)
    ...
    # Static shared memory buffer for tiled processing (one BLOCK of float32)
    shared_buf = tl.static_shared_memory(shape=(BLOCK,), dtype=tl.float32)
    ...
    for m in range(m_start, m_end):
        for group_idx in range(num_groups_per_row):
            ...
            if actual_group_size <= BLOCK:
                # ---- Fused small group: one load, compute, store ----
                ...
                # ---- Tiled large group: one HBM read, buffer in shared memory ----
                _absmax = eps.to(tl.float32)

                # First sweep: load tiles, compute partial max, store to shared memory
                for chunk_start in range(0, actual_group_size, BLOCK):
                    ...
                    # Buffer the tile in shared memory
                    tl.store(shared_buf + cols, y_chunk, mask=mask)
                ...
                # Compute scale from global max
                y_s = _absmax / bit8_max
                if SCALE_UE8M0:
                    y_s = tl.exp2(tl.ceil(tl.log2(tl.abs(y_s))))

                # Second sweep: read from shared memory, quantize, store output
                for chunk_start in range(0, actual_group_size, BLOCK):
                    chunk_end = min(chunk_start + BLOCK, actual_group_size)
                    chunk_size = chunk_end - chunk_start
                    cols = tl.arange(0, BLOCK)
                    mask = cols < chunk_size
                    y_chunk = tl.load(shared_buf + cols, mask=mask, other=0.0).to(tl.float32)
                    y_q_chunk = tl.clamp(y_chunk / y_s, bit8_min, bit8_max).to(y_q_ptr.dtype.element_ty)
...
def per_token_group_quant_8bit_colmajor(y: torch.Tensor, group_size: int, eps: float = 1e-5, scale_ue8m0: bool = False):
    ...
    # Compute BLOCK: smallest multiple of 16 >= group_size, capped at 1024 to avoid resource spill.
    # For groups > 1024, tiled shared-memory path is used.
    BLOCK = min(((group_size + 15) // 16) * 16, 1024)
