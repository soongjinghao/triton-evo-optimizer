...
def fn_triton_kernel(
    ...
    pid = tl.program_id(0)
    token_block = pid // num_head_blocks
    head_block = pid % num_head_blocks
    token_id = token_block * BLOCK_ROWS + tl.arange(0, BLOCK_ROWS)
    head_id = head_block * BLOCK_HEADS + tl.arange(0, BLOCK_HEADS)
...
def fn_triton(k, k_nope, k_rope, qk_nope_head_dim, qk_rope_head_dim, num_local_heads):
    ...
    num_token_blocks = triton.cdiv(num_tokens, 8)
    num_head_blocks = triton.cdiv(num_local_heads, 32)
    grid = (num_token_blocks * num_head_blocks,)
