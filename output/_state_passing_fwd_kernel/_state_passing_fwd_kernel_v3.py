...
def _state_passing_fwd_kernel(
    ...
    不固定 batch/nchunks/nheads/dim，也不假设只有一个测试形状。
    主要优化：
      1. shape、stride 全部作为 tl.constexpr；
      2. 编译期删除无关分支；
      3. HAS_INITSTATES=True 且非 continuous batching 时，
         seq_idx 不影响计算，完全删除对应加载；
      4. BLOCK_SIZE 由 dim 动态选择，而不是固定 128；
      5. 使用较小的 num_warps/num_stages。
    """
    ...
    final_base = (
        final_states_ptr
        + pid_b * STRIDE_FINAL_BATCH
        + pid_h * STRIDE_FINAL_HEAD
    )
    dA_base = (
        dA_cs_ptr
    ...
    # NCHUNKS 是编译期常量，后端可以消除循环中的静态地址计算。
    for c in range(NCHUNKS):
        ...
        dA = tl.load(dA_ptr).to(tl.float32)
        ...
        if NEEDS_CONT_SEQ:
            ...
            if seq_changed:
                ...
                seq_idx_chunk_start = tl.load(
                    seq_base
                    + seq_start_idx * STRIDE_SEQ_SEQLEN
                ).to(tl.int32)
...
def _state_passing_fwd(
    ...
    # 不固定具体测试 shape，只根据 dim 选择合理 block。
    block_size = min(
        256,
