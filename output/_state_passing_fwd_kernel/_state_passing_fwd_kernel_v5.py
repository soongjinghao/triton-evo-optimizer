...
def _state_passing_fwd_kernel(
    ...
    states_base = tl.multiple_of(
        states_ptr + pid_b * STRIDE_STATES_BATCH + pid_h * STRIDE_STATES_HEAD,
        16,
    )
    out_base = tl.multiple_of(
        out_ptr + pid_b * STRIDE_OUT_BATCH + pid_h * STRIDE_OUT_HEAD,
        16,
    )
    final_base = tl.multiple_of(
        final_states_ptr + pid_b * STRIDE_FINAL_BATCH + pid_h * STRIDE_FINAL_HEAD,
        16,
    )
    dA_base = tl.multiple_of(
        dA_cs_ptr + pid_b * STRIDE_DA_BATCH + pid_h * STRIDE_DA_HEAD
    ...
    if HAS_INITSTATES:
        ...
        state = tl.load(
            init_base + offs * STRIDE_INIT_DIM,
            mask=mask,
            other=0.0,
            cache_modifier=".ca",
        ).to(tl.float32)
    ...
    for c in range(NCHUNKS):
        ...
        dA = tl.load(dA_ptr, cache_modifier=".ca").to(tl.float32)
        ...
        if NEEDS_CONT_SEQ:
            ...
            if seq_changed:
                init_base2 = tl.multiple_of(
                    initstates_ptr + seq_idx_chunk_end * STRIDE_INIT_BATCH
                    + pid_h * STRIDE_INIT_HEAD,
                    16,
                )
                state = tl.load(
                    init_base2 + offs * STRIDE_INIT_DIM,
                    mask=mask,
                    other=0.0,
                    cache_modifier=".ca",
                ).to(tl.float32)
                ...
                logical_chunk_idx += (
                    seq_idx_chunk_end - seq_idx_chunk_start
                )
            ...
            scale_mask = (
                seq_idx_chunk_end == prev_seq_idx_chunk_end
            )
            prev_seq_idx_chunk_end = seq_idx_chunk_end
...
def _state_passing_fwd(
    ...
    block_size = min(
        256,
        triton.next_power_of_2(max(1, dim)),
    )
    if block_size <= 32:
        num_warps = 1
    elif block_size <= 128:
