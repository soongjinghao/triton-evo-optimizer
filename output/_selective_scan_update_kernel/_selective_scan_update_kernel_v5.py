...
@triton.heuristics({"BLOCK_SIZE_DSTATE": lambda args: 16 if args["dstate"] <= 16 else 32 if args["dstate"] <= 32 else 64 if args["dstate"] <= 64 else 128})
@triton.jit
def _selective_scan_update_kernel(
    ...
    # Unified mask for state and A loads/stores (reused to avoid recomputation)
...
def selective_state_update(
    ...
    # BLOCK_SIZE_M must be a multiple of 16 for Ascend NPU, dynamically chosen by dstate size
    if dstate <= 16:
        BLOCK_SIZE_M = 32
        num_warps = 4
    elif dstate <= 32:
