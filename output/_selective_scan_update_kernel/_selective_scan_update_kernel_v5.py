...
def _selective_scan_update_kernel(
    ...
    state_ptrs = state_ptr + offs_m[:, None] * stride_state_dim + offs_n[None, :]
    x_ptrs = x_ptr + offs_m
    dt_ptrs = dt_ptr + offs_m
    if HAS_DT_BIAS:
        dt_bias_ptrs = dt_bias_ptr + offs_m
    ...
    # 加载 state
    if HAS_STATE_BATCH_INDICES:
        state_mask = mask_2d & (state_batch_idx != pad_slot_id)
    else:
        state_mask = mask_2d
...
def selective_state_update(
    ...
    assert state.stride(-1) == 1, "state must be contiguous in the last dim"
    assert x.stride(-1) == 1, "x must be contiguous in the last dim"
    assert dt.stride(-1) == 1, "dt must be contiguous in the last dim"
    assert A.stride(-1) == 1, "A must be contiguous in the last dim"
    assert B.stride(-1) == 1, "B must be contiguous in the last dim"
    assert C.stride(-1) == 1, "C must be contiguous in the last dim"
    if D is not None:
        assert D.stride(-1) == 1, "D must be contiguous in the last dim"
    if z is not None:
        assert z.stride(-1) == 1, "z must be contiguous in the last dim"
    if dt_bias is not None:
        assert dt_bias.stride(-1) == 1, "dt_bias must be contiguous in the last dim"
    assert out.stride(-1) == 1, "out must be contiguous in the last dim"
