...
def _selective_scan_update_kernel(
    ...
    # Base mask for dimension bounds, reused for state and A loads/stores
    base_mask = (offs_m[:, None] < dim) & (offs_n[None, :] < dstate)
    mask_state = base_mask
    if HAS_STATE_BATCH_INDICES:
    ...
    if not TIE_HDIM:
        ...
        if DT_SOFTPLUS:
            dt = softplus(dt)
        A = tl.load(A_ptrs, mask=base_mask, other=0.0).to(tl.float32)
        dA = tl.exp(A * dt[:, None])
    else:
...
def selective_state_update(
    ...
    # Dynamic BLOCK_SIZE_M selection based on dstate (same as faster Parent 1)
    if dstate <= 16:
        BLOCK_SIZE_M = 32
        num_warps = 4
    elif dstate <= 32:
