...
def _act_quant_kernel(
    X_ptr,
    Y_ptr,
    S_ptr,  # points to (num_groups, M) layout
    M,
    N,
    ...
    # Store scales in transposed layout: (num_groups, M)
    # Each group writes a contiguous block of M elements
...
def act_quant(
    ...
    # Store scales in transposed layout (num_groups, M) for coalesced writes
    num_groups = N // block_size
    s_transposed = torch.empty(num_groups, M, dtype=torch.float32, device=x.device)

    # Adaptive BLOCK_M: use 64 for better occupancy, but ensure multiple of 16
    ...
    # Transpose to final shape (M, num_groups) as expected by caller
