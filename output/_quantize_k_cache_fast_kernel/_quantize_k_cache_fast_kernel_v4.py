...
def _quantize_k_cache_fast_kernel(
    ...
    token_id = block_id // TOTAL_BLOCKS
    raw_block_id = block_id % TOTAL_BLOCKS

    if raw_block_id < NOPE_GRID_BLOCKS:
        effective_block_id = raw_block_id * 4
        ...
        offs_s = effective_block_id + tl.arange(0, 4)
        ...
        effective_block_id = (raw_block_id - NOPE_GRID_BLOCKS) * 4
        offs = effective_block_id * GROUP_SIZE + tl.arange(0, BLOCK_N)
        mask = offs < DIM_ROPE
...
def _quantize_k_cache_fast(k_nope, k_rope, group_size: int = 128):
    ...
    grid = (min(num_tokens * total_blocks, 65535),)
