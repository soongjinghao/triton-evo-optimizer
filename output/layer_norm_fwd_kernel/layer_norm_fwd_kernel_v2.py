...
def layer_norm_fwd_kernel(
    ...
    if N > BLOCK_N:
        ...
        # First pass: full blocks
        for block_idx in range(num_full_blocks):
            col_start = block_idx * BLOCK_N
            cols = col_start + tl.arange(0, BLOCK_N)
            mask = row_mask[:, None]  # col mask always true
        ...
        for block_idx in range(num_full_blocks):
            ...
            w = tl.load(W + w_offsets, mask=None, other=0.0).to(tl.float32)
            if HAS_BIAS:
                b = tl.load(B + w_offsets, mask=None, other=0.0).to(tl.float32)
            if not IS_RMS_NORM:
