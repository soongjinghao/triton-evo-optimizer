...
def _per_group_transpose(
    ...
    tl.multiple_of(data_start_ptr, 16)
    tl.multiple_of(trans_data_start_ptr, 16)
    ...
    if k % BLOCK_SIZE_K == 0:
        for start_m in tl.range(0, num_tokens_of_expert, BLOCK_SIZE_M * tl.num_programs(1)):
            ...
            data = tl.load(data_start_ptr + off, mask=m_mask[:, None], other=0.0)
            tl.store(trans_data_start_ptr + trans_off, data, mask=m_mask[:, None])
    else:
        for start_m in tl.range(0, num_tokens_of_expert, BLOCK_SIZE_M * tl.num_programs(1)):
            m_coord = start_m + m_id * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
            m_mask = m_coord < num_tokens_of_expert
...
def per_group_transpose(
    ...
    _per_group_transpose[grid](
        ...
        BLOCK_SIZE_M=16, BLOCK_SIZE_K=8
    )
    return trans_a