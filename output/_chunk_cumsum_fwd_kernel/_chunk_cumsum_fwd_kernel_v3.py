...
def _chunk_cumsum_fwd_kernel_aligned(
    ...
    dt = tl.minimum(tl.maximum(dt, dt_min), dt_max)
    tl.store(
        tl.multiple_of(dt_out_base, 16) + offs_h[:, None] * stride_dt_out_head + offs_c[None, :],
        dt
    )
    A = tl.load(
        tl.multiple_of(A_ptr, 16) + offs_h * stride_A_head
    ...
    dA_cs = tl.cumsum(dA, axis=1)
    tl.store(
        tl.multiple_of(dA_cumsum_base, 16) + offs_h[:, None] * stride_dA_cs_head + offs_c[None, :],
        dA_cs
    )
