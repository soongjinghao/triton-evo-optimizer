...
def _chunk_cumsum_fwd_kernel(
    ...
    if ALIGNED:
        dt = tl.load(dt_ptrs).to(tl.float32)
        A = tl.load(A_ptrs).to(tl.float32)
        if HAS_DT_BIAS:
            dt_bias = tl.load(dt_bias_ptr + offs_h * stride_dt_bias_head).to(tl.float32)
            dt += dt_bias[:, None]
        if DT_SOFTPLUS:
            dt = tl.where(dt <= 20.0, softplus(dt), dt)
        dt = tl.clamp(dt, dt_min, dt_max)
        dt_out_ptrs = dt_out_ptr + offs_h[:, None] * stride_dt_out_head + offs_c[None, :]
        tl.store(dt_out_ptrs, dt)
        dA = dt * A[:, None]
        dA_cs = tl.cumsum(dA, axis=1)
        dA_cs_ptrs = dA_cumsum_ptr + offs_h[:, None] * stride_dA_cs_head + offs_c[None, :]
        ...
        dt = tl.load(dt_ptrs, mask=full_mask, other=0.0).to(tl.float32)
        if HAS_DT_BIAS:
            dt_bias = tl.load(dt_bias_ptr + offs_h * stride_dt_bias_head, mask=h_mask, other=0.0).to(tl.float32)
            dt += dt_bias[:, None]
        if DT_SOFTPLUS:
            dt = tl.where(dt <= 20.0, softplus(dt), dt)
        dt = tl.clamp(dt, dt_min, dt_max)
        ...
        dt_out_ptrs = dt_out_ptr + offs_h[:, None] * stride_dt_out_head + offs_c[None, :]
        tl.store(dt_out_ptrs, dt, mask=h_mask[:, None] & (offs_c[None, :] < chunk_size))
        A = tl.load(A_ptrs, mask=h_mask, other=0.0).to(tl.float32)
        dA = dt * A[:, None]
        dA_cs = tl.cumsum(dA, axis=1)
        dA_cs_ptrs = dA_cumsum_ptr + offs_h[:, None] * stride_dA_cs_head + offs_c[None, :]
        tl.store(dA_cs_ptrs, dA_cs, mask=h_mask[:, None] & (offs_c[None, :] < chunk_size))
...
def _chunk_cumsum_fwd(
    ...
    aligned = is_pow2 and heads_aligned and full_chunks
