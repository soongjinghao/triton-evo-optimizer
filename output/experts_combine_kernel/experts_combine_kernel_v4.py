...
def experts_combine_kernel(
    ...
    moe_ptr = moe_hidden_states + start_index_rmoe + combine_k_offsets[:, None] * hidden_dim + offsets[None, :]
    moe_x = tl.load(moe_ptr, mask=mask[None, :], other=0.0)
    moe_x = tl.sum(moe_x, axis=0)

    mlp_ptr = mlp_hidden_states + start_index_mlp + offsets
    mlp_x = tl.load(mlp_ptr, mask=mask, other=0.0)
