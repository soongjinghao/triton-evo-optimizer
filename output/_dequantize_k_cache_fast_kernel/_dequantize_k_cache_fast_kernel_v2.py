...
def _dequantize_k_cache_fast_kernel(
    ...
    # 1. dequant nope: 512 int8 * 4 float32 scales -> bf16
    offs_nope = tl.arange(0, DIM_NOPE)
    ptr_q = tl.multiple_of(
        input_nope_q_ptr + token_id * input_nope_q_stride_0, 16
    ) + offs_nope
    q = tl.load(ptr_q).to(tl.float32)

    offs_s = tl.arange(0, NUM_NOPE_BLOCKS)
    ptr_s = tl.multiple_of(
        input_nope_s_ptr + token_id * input_nope_s_stride_0, 16
    ) + offs_s
    ...
    ptr_out_nope = tl.multiple_of(
        output_ptr + token_id * output_stride_0, 16
    ) + offs_nope
    tl.store(ptr_out_nope, y_nope)

    # 2. copy rope: 64 bf16
    offs_rope = tl.arange(0, DIM_ROPE)
    ptr_rope = tl.multiple_of(
        input_rope_ptr + token_id * input_rope_stride_0, 16
    ) + offs_rope
    rope = tl.load(ptr_rope).to(tl.bfloat16)

    offs_out_rope = DIM_NOPE + offs_rope
    ptr_out_rope = tl.multiple_of(
        output_ptr + token_id * output_stride_0, 16
    ) + offs_out_rope
