...
def merge_16x16_to_32x32_inverse_kernel(
    ...
    # Two-loop fusion with pre-computed base addresses
    base1 = A + (i_t * BT + 2) * H * BT
    base2 = A + (i_t * BT + 18) * H * BT
    ...
    for k in range(14):
        if k + 2 < limit1:
            ...
            b_Ai_11 = tl.where((o_i == k + 2)[:, None], b_a_11, b_Ai_11)
            ptr1 += H * BT
        if k + 18 < limit2:
            b_a_22 = -tl.load(ptr2)
            b_a_22 += tl.sum(b_a_22[:, None] * b_Ai_22, 0)
            b_Ai_22 = tl.where((o_i == k + 2)[:, None], b_a_22, b_Ai_22)
            ptr2 += H * BT
